...
Na validação pública com cache busting, o CSS e as fontes foram carregados: `Cormorant Garamond` e `Manrope` retornam `document.fonts.check(...) = true`; o título mede `108.8px` em viewport desktop de 1280px. O preview local desktop permanece visualmente equivalente. No viewport mobile de 390px, o hero agora usa 20vw com mínimo de 4.6rem e o copy foi ampliado para .9rem/1.75. O HTML também foi ajustado para `viewport-fit=cover` e `-webkit-text-size-adjust: 100%`.

Validação externa em `https://gborges1441-tech.github.io/atelier-nova-salao/?v=f1123e3`: a URL pública carregou o título Atelier Nova, o motion story e o novo rail de capítulos (`A matéria`, `O acabamento`, `A assinatura`). O build do Pages ficou `built` no commit `f1123e3`; o HTML serviu o conteúdo completo sem 404.
